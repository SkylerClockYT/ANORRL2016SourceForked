...
bool AuthenticationHelper::eventFilter(QObject * watched, QEvent * evt)
{
	...
	{
        // save webkit cookies
		savedCookieKey = RobloxNetworkAccessManager::Instance().cookieJar()->getCookieValue(RobloxSettings::getBaseURL(), "ANORRLSECURITY");
	}
	else if (eventType == QEvent::ApplicationActivate)
	{
		QString currentCookieKey = RobloxNetworkAccessManager::Instance().cookieJar()->getCookieValue(RobloxSettings::getBaseURL(), "ANORRLSECURITY");
...
void AuthenticationHelper::authenticateUserAsync(const QString& url, const QString& ticket)
{
	...
	else
	{
		...
		QString secCookie = RobloxNetworkAccessManager::Instance().cookieJar()->getCookieValue(RobloxSettings::getBaseURL(), "ANORRLSECURITY");
