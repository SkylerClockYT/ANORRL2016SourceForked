...
void Application::UploadSessionLogs()
{
	if(!logManager.CreateFakeCrashDump())
	{
		...
		MessageBoxA(mainWindow, 
			"Logfiles will be uploaded next time you start ANORRL. Please restart ANORRL now.", 
			"ANORRL", MB_OK);
	}
}
...
static void doMachineIdCheck(Application* app, FunctionMarshaller* marshaller, HWND hWnd) {
	if (MachineIdUploader::uploadMachineId(GetBaseURL().c_str()) ==
		...
		MessageBoxA(hWnd, MachineIdUploader::kBannedMachineMessage, "ANORRL", MB_OK);
...
bool Application::LoadAppSettings(HINSTANCE hInstance)
{
	try {
		...
		CRegKey key;
		bool validRegKey = (key.Open(HKEY_CURRENT_USER, "Software\\GraceRBLX\\ANORRL", KEY_READ) == ERROR_SUCCESS);
		if (validRegKey)
		{
			DWORD value = TRUE;
		...
		case RBX::DebugSettings::Prompt: 
			{
				...
				if (MessageBoxA(mainWindow, message, "ANORRL", MB_YESNO)==IDYES)
					dumpErrorUploader->Upload(dmpHandlerUrl);
				else
...
void Application::doOpenUrl(const std::string url)
{
	...
	if (HWND dialog = webView->Create( mainWindow, SW_SHOWNORMAL))
		...
		MessageBox( mainWindow, "There was a problem opening the in-game browser.", "ANORRL", MB_OK | MB_ICONSTOP);
}
