	...
	, parentView(view)
{
	...
	if( FAILED( hr = DirectInput8Create(GetModuleHandle(NULL),
										...
										NULL ) ) )
	{
		MessageBoxA(NULL, "ANORRL could not initialize input. Please make sure you have a mouse and keyboard plugged in.", "ANORRL", MB_OK);
		return;
	}
...
void UserInput::createMouse()
{
	...
	{
		MessageBoxA(NULL, "Could not find a mouse! ANORRL requires a mouse.", "ANORRL", MB_OK);
		return;
	}

	if (FAILED(hr = diMousePtr->SetDataFormat(&c_dfDIMouse2)))
	{
		MessageBoxA(NULL, "Could not find a mouse (data format failed)! ANORRL requires a mouse.", "ANORRL", MB_OK);
		return;
	}
	...
	{
		MessageBoxA(NULL, "Could not find a mouse (set property failed)! ANORRL requires a mouse.", "ANORRL", MB_OK);
		return;
	}
}
...
void UserInput::createKeyboard()
{
	...
	{
		MessageBoxA(NULL, "Could not find a keyboard! ANORRL requires a keyboard.", "ANORRL", MB_OK);
		return;
	}

	if (FAILED(hr = diKeyboardPtr->SetDataFormat(&c_dfDIKeyboard)))
	{
		MessageBoxA(NULL, "Could not find a keyboard (data format failed)! ANORRL requires a keyboard.", "ANORRL", MB_OK);
		return;
	}
	...
	{
		MessageBoxA(NULL, "Could not find a keyboard (set property failed)! ANORRL requires a keyboard.", "ANORRL", MB_OK);
		return;
	}
}
